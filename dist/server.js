...
app.post("/player-statistic-update", (req, res) => {
    ...
    let players = game.players;
    const index = game.players.findIndex(player => player.playerID === playerID);
    if (index >= 0) {
...
app.post("/player-add", (req, res) => {
    try {
        ...
        const player = { name, password, totalRounds: 0, totalFlip: 0, timeStatist: 0, playerID: helpers_1.default() };
